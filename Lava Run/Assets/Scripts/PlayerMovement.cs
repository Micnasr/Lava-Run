...
public class PlayerMovement : MonoBehaviour {
    ...
    private void MyInput() {
        ...
        //Crouching
        if (Input.GetKeyDown(KeyCode.LeftControl) && (gameObject.transform.position.y > -20))
            StartCrouch();
    ...
    void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.tag == "elevator")
        {
            ...
            if (elevator == false)
            {
                leftDoor.SetBool("Closed", true);
                rightDoor.SetBool("Closed", true);
